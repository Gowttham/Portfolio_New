...
// Then add them to frontend/.env  (prefix with VITE_ so Vite exposes them):
...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // EmailJS sends the form directly from the browser — no backend needed.
      // The template variables {{name}}, {{email}}, {{message}} must match
      // the field names in your EmailJS template.
      await emailjs.sendForm(SERVICE_ID, TEMPLATE_ID, formRef.current, {
        publicKey: PUBLIC_KEY,
      })
      setStatus({ msg: 'Message sent successfully! I\'ll get back to you soon.', type: 'ok' })
      setForm({ name: '', email: '', message: '' })
    } catch (err) {
      console.error('EmailJS error:', err)
      setStatus({ msg: 'Something went wrong. Please try again or email me directly.', type: 'err' })
    } finally {
      setLoading(false)
